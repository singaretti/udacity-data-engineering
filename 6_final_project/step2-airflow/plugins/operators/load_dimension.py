...
class LoadDimensionOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        if self.append_mode:
            ...
            truncate_table = f"TRUNCATE {self.table_name}_tmp"
            PostgresHook(postgres_conn_id=self.redshift_conn_id).run(truncate_table)
        
        else:
            self.log.info(f"Append Mode OFF: Truncate & Insert statements in {self.table_name} dimension table")
            PostgresHook(postgres_conn_id=self.redshift_conn_id).run(f"TRUNCATE {self.table_name};")
